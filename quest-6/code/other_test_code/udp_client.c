...
#define TIMER_INTERVAL2_SEC   (1)    // Sample test interval for the first timer
...
void IRAM_ATTR splitTimer_isr() {
    TIMERG1.int_clr_timers.t1 = 1;  // clear timer
    flag = 1;
}

static void splitTimer_initialize() {
    ...
    timer_set_alarm_value(TIMER_GROUP_1, TIMER_1, TIMER_INTERVAL2_SEC * TIMER_SCALE);
    timer_enable_intr(TIMER_GROUP_1, TIMER_1);
    timer_isr_register(TIMER_GROUP_1, TIMER_1, splitTimer_isr, (void*)TIMER_1, ESP_INTR_FLAG_IRAM, NULL);

    // Start timer
    timer_start(TIMER_GROUP_1, TIMER_1);
}
void writeTimeToDisplay(char secStr[20], char minStr[20]) {
    int sec_first;
    int sec_second;
    int min_first;
    int min_second;
...
void tickTime() {
    ...
    if (secCount > 58) {
        ...
        sprintf(sec_int, "%d", secCount);
        sprintf(min_int, "%d", minCount);
        writeTimeToDisplay(sec_int, min_int);
...
static void split_timer_task(void *pvParameters)
{
	while (1) {
			if (flag == 1) {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(example_connect());
		...
		splitTimer_initialize();

		writeTimeToDisplay(secStr, minStr);


    xTaskCreate(udp_client_task, "udp_client", 4096, NULL, 5, NULL);
		xTaskCreate(split_timer_task, "split_timer_task", 4096, NULL, 5, NULL);
}
