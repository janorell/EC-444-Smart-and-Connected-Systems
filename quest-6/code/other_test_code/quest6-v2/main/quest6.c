...
void writeTimeToDisplay(char secStr[20], char minStr[20]) {
    ...
    displaybuffer[1] = 0b0000110000111111;
    displaybuffer[2] = 0b0000110000111111;
...
void collision_task(){
	while(1){
      ...
      // backwards until too close to back wall
      while (distance_IR3 >= .8 && distCollision <= 90) {
        motorTurn(700); //hard right
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        motorDrive(1550);
      }
      ...
      //perform until sensors are even and disCollision is farther than say 100cm
			...
			//motorDrive();
			//motorBackward();
		} else if (distCollision <= 275) { // start turning
      printf("Starting left turn \n");
      motorTurn(2300); // hard left
      // motorDrive((int)(driveSpeed + (1400 - driveSpeed)/2)); // slight slowdown
      motorDrive(driveSpeed);
      ...
      // if (driveSpeed > 1200 && driveSpeed < 1300) {
      //   driveSpeed -= 25;
      // }
